...
import sys
import tqdm
# from typing import Sequence, ValuesView, final
import h5py
import time
import datetime
...
parser.add_argument('--save-name', type=str, default='Fraction.mp4', help="video name to save (ends with .mp4)")
...
def main():
    dataset = ['./dataset/video1.h5','./dataset/video2.h5','./dataset/video3.h5','./dataset/video4.h5','./dataset/video5.h5','./dataset/video6.h5','./dataset/video7.h5','./dataset/video8.h5']
    summary_list = []
    combined_sum = []
    ...
    frm_dir = ["./data/video01","./data/video02","./data/video03","./data/video04","./data/video05","./data/video06","./data/video07","./data/video08"]
    for index, item in enumerate(frm_dir):
        print(summary_list[index])
        
        combined_sum.append(frm2video(summary_list[index]))
    ...
    # print(bb)
    c = combined_sum[0]
    cc = dict.fromkeys(c, "v1")
    # print(cc)
    e = combined_sum[1]
    ee = dict.fromkeys(e, "v2")
    # print(ee)
    f = combined_sum[0]
    ff = dict.fromkeys(f, "v1")
    # print(ff)
    g = combined_sum[1]
    gg = dict.fromkeys(g, "v2")
    # print(gg)
    h = combined_sum[0]
    hh = dict.fromkeys(h, "v1")
    # print(hh)
    i = combined_sum[1]
    ii = dict.fromkeys(i, "v2")
    # print(ii)

    dd = defaultdict(list)
    for d in (aa, bb ,cc ,ee,ff,gg,hh,ii):                   # you can list as many input dicts as you want here
    ...
    for k,v in dd.items():
        ...
        for i in range(len(v)):
            # print(k,v[i])
            # print(k,v[i])
            frame_path = summary.append(os.getcwd()+"/"+"data" +"/" + str(v[i])+ "/" + str(k) + '.jpg')
    print(summary)
